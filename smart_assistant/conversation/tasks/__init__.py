...
# from playsound import playsound FIXME fix playsound
# from gtts import gTTS FIXME fix gtts
# import speech_recognition as sr FIXME fix speech_recognition

from smart_assistant.conversation.tasks import uwg
from smart_assistant.conversation.tasks import utils
...
MODEL = load_model(f'{current_directory}/smart_assistant/conversation/tasks/models/chatbot_model.h5')

INTENTS = json.load(open(f'{current_directory}/smart_assistant/conversation/tasks/models/intents.json', encoding='UTF-8'))
WORDS = pickle.load(open(f'{current_directory}/smart_assistant/conversation/tasks/models/words.pkl', 'rb'))
CLASSES = pickle.load(open(f'{current_directory}/smart_assistant/conversation/tasks/models/classes.pkl', 'rb'))

LIST_EVENTS = json.load(open(f'{current_directory}/smart_assistant/conversation/tasks/uwg/Events.json', encoding='UTF-8'))
...
with open(f'{current_directory}/smart_assistant/conversation/tasks/uwg/students.txt', 'r', encoding='UTF-8') as filehandle:
...
def format_people(people):
    """
    Formats the people list into a string.

    :param people: The list of people to format.
    :return: A string of the people.
    """
    ...
    if tag == "time":
        ...
        for i in list_of_intents:
            if i['tag'] == tag:
                ...
                if "{}" in response:
                    response = response.format(format_people(people))
    ...
    if start != "":
        ...
        if start_tag == 'greeting':
            ...
            if any("Unknown" in element for element in people):
                # FIXME Fix this to be a random from responses
                introduction = "Hello, I dont think I know you."
            else:
                names = format_people(people)
